...
export const SaleReportPDF = async (e, item, user, extra) => {
    const {
    ...
    doc.setFontSize(10).setTextColor(51, 51, 51).setFont("helvetica", 'normal').text(Shop, marginLeft + 94, marginTop + 40)
    ...
    doc.setFontSize(16).setTextColor(0, 0, 0).setFont('helvetica', 'normal').text(ReportTitle, doc.internal.pageSize.getWidth() / 2, 91, { align: "center" });
    ...
    let INV_ProductItems = item.map((row) => {
        if (row.total_grand.Count) {
            let rows = [
                [{ Title: "COUNTER NO - " + row.No, value: row.No }],
                [
                    { Title: "Total Invoice", value: parseFloat(row.total_grand.Count).toLocaleString('en', { useGrouping: true, minimumFractionDigits: 0 }) },
                    { Title: "Sale Total", value: parseFloat(row.total_grand.GrandTotal).toLocaleString('en', { useGrouping: true, minimumFractionDigits: 2 }) },
                    ...
                    { Title: "Payment", value: parseFloat(row.total_grand.Payment).toLocaleString('en', { useGrouping: true, minimumFractionDigits: 2 }) },
            ...
            if (extra.ModeFilter.value !== 1) {
                rows[1].splice(2, 0, { Title: "Due", value: parseFloat(row.total_grand.Due).toLocaleString('en', { useGrouping: true, minimumFractionDigits: 2 }) });
            }
            if (No === 1 && Scale === 1) {
                rows[1].unshift({ Title: row.SisterTitle + " (" + row.SectorTitle + ")", value: 0 });
                rows[1].push(
                    { Title: "COGS", value: parseFloat(row.total_grand.Cost).toLocaleString('en', { useGrouping: true, minimumFractionDigits: 2 }) },
                    { Title: "Revenue", value: parseFloat((parseFloat(row.total_grand.GrandTotal) - parseFloat(row.total_grand.RefundAmount)) - (parseFloat(row.total_grand.Cost) + parseFloat(row.total_grand.Discount))).toLocaleString('en', { useGrouping: true, minimumFractionDigits: 2 }) }
                );
            }
            return rows;
        } else {
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        if (i !== 1) {
            ...
            doc.setFontSize(20);
            doc.setTextColor(40);
            doc.setFontSize(12).setFont("helvetica", 'bold').text(CollocationTitle, 40, 40, { align: "left" })
        ...
        if (i === pageCount) {
            ...
            // doc.setFontSize(11).setTextColor(128, 128, 128).setFont("helvetica", 'normal').text(RoleTitle, 260, pageHeight - 60, { align: "center" })
            // doc.setFontSize(11).setTextColor(0, 0, 0).setFont("helvetica", 'bold').text(Name, 260, pageHeight - 70, { align: "center" })
