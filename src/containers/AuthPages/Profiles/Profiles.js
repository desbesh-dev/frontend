...
import { FetchUsrLadger, LoadProfile } from '../../../actions/APIHandler';
import { logout } from '../../../actions/auth';
import { DISPLAY_OVERLAY } from '../../../actions/types';
import { customHeader, locales } from '../../../hocs/Class/datepicker';
let today = new Date();

const Profiles = ({ user, UserID, list, setList, no }) => {
    ...
    const BisLadger = async () => {
        ...
        var result = await FetchUsrLadger(UserID, date_from, date_to);
        if (result !== true)
            setData(result.data);
        dispatch({ type: DISPLAY_OVERLAY, payload: false });
    ...
    const PrvDate = (i) => Array.isArray(UserData) && UserData.length ? typeof (UserData[i - 1]) !== 'undefined' && UserData[i - 1] != null ? UserData[i - 1].Date : 0 : 0
                    ...
                    UserData ?
                        <div className="row bg-white mb-2">
                            <div className="col-md-2 text-center d-flex justify-content-center p-2">
                                <img
                                    src={UserData?.avatar ?? process.env.REACT_APP_API_URL + "/Media/no_logo.jpeg"}
                            ...
                            <div className="col-md-4 text-md-left my-auto">
                                <p className="fs-5 m-0">
                                    <p className="fs-4 fw-bold m-0">{UserData?.Name} <small className="text-muted fw-bold m-0">{UserData?.username}</small></p>
                                    <p className="fs-5 fw-bold m-0">{UserData?.DetailID?.MobileNo + ", " + UserData?.DetailID?.email}</p>
                                    <p className="fs-5 fw-normal m-0">{UserData?.DetailID?.PresentAddress}</p>
                                </p>
                            </div>
                            ...
                            <div className="col-md-5 text-md-right my-auto">
                                <p className="fs-5 m-0">
                                    <p className="fs-4 fw-bold m-0">{UserData?.SectorID?.SisterID.Title}</p>
                                    <p className="fs-5 fw-bold m-0">{UserData?.SectorID?.SectorTitle + " (" + UserData?.SectorID?.SisterID?.ShortCode + ")"}</p>
                                    <p className="text-muted fw-bold m-0">{UserData?.Role?.Title}</p>
                                </p>
                                {
                                    no === 7 ?
                                        <button className="btn fs-6 fw-bold text-dark" onClick={() => FetchUser(UserData.id.replace(/-/gi, ''))}><i className="fad fa-edit" /> Update</button>
                                        : null
                                }
                            </div>
                        </div>
