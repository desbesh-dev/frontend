...
const UserList = ({ display, user, scale, sub_scale, auth }) => {
    ...
    FilterProfiles = Array.isArray(Data) && Data.length ? Data.filter(function (item) {
        ...
        return BothValue
    }).map(function ({ id, BranchID, BranchName, CompanyID, CompanyName, Cond, IssueDate, FarmID, Sell, FullName, UserID, Status, details }) {
        return { id, BranchID, BranchName, CompanyID, CompanyName, Cond, IssueDate, FarmID, Sell, FullName, UserID, Status, details };
    }) : null
    let unique_search = Array.isArray(FilterProfiles) && FilterProfiles.length ? findUnique(FilterProfiles, d => d.FullName) : null;

    const LogoutUser = async (id) => {
        const result = await ForceLogoutUser(auth.refresh)
    }

    const LogoutAllDevice = async () => {
        const result = await LogOutAllDevice()
    }
                    ...
                    <div id="products" className="row view-group m-0 p-0" style={{ display: View ? 'flex' : null }}>
                        {
                            Array.isArray(FilterProfiles) && FilterProfiles.length ? FilterProfiles.map((item, i) => (
                                <div className={View ? "item col-xs-3 col-lg-3 grid-group-item mb-3" : "item col-xs-3 col-lg-3 grid-group-item list-group-item mb-3"} key={i}>
                                    <Link to={`${item.Status === 1 ? `/user_profile/${item.id}` : `/employee_main/${item.id}`}`}
                                        ...
                                        <div className="caption card-body d-flex flex-column justify-content-center py-0 px-2 h-100">
                                            <p className="group inner list-group-item-text fs-5 m-0">
                                                <p className="group card-title inner list-group-item-text fs-4 fw-bold m-0">{item.details.FullName}</p>

                                                <small className="group card-title inner list-group-item-text m-0">{item.CompanyID + ". " + item.CompanyName}</small>

                                                <p className="fs-5 fw-bold m-0"> {item.Status === 2 ? "Staff " : item.Status === 1 ? "Subscriber" : null}</p>

                                                <div className='d-flex justify-content-start align-items-center'>
                                                    <small className="fw-bold text-muted m-0"> {"USER ID: " + item.id}</small>
                                                    <button className="btn fw-bold text-warning m-0" onClick={(e) => { e.preventDefault(); LogoutUser(item.id) }}> Log out</button>
                                                    <button className="btn fw-bold text-danger m-0" onClick={(e) => { e.preventDefault(); LogoutAllDevice() }}> Log out all</button>
                                                </div>
                                            </p>
                                        </div>
