...
import { FetchDueInvoices } from '../../../actions/PartyAPI';
import { DISPLAY_OVERLAY } from '../../../actions/types';
import { customHeader, locales } from "../../Suppliers/Class/datepicker";
import { DueInvoiceListPDF } from './DueInvoiceListPDF';
let today = new Date();
...
const DueInvoices = ({ data, no }) => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/due_invoice_list/`, {
                headers: {
    ...
    const LoadDueInvoices = async (e) => {
        ...
        var result = await FetchDueInvoices(moment(DateFrom).format("YYYY-MM-DD"), moment(DateTo).format("YYYY-MM-DD"), SisterFilter, SectorFilter, sharpnessValue, OrderBy);

        if (result !== true)
            DueInvoiceListPDF(e, result.data, false, DateFrom, DateTo, data.Name, SisterFilter?.label + " (" + SectorFilter?.label + ")", sharpnessValue?.label, OrderBy?.label)
            ...
            <div className="col-md-12 justify-content-center align-items-center h-100 p-0">
                <div className={`d-flex justify-content-between bg-white py-2 px-2`}>
                    ...
                    <div className="d-flex">
                        <p className='display-6 bg-white fw-bolder m-0 text-nowrap pr-3'>PAYMENT DUE REPORT</p>
                        <Datepicker
                            selected={DateFrom}
                                ...
                                <tbody>
                                    {
                                        Array.isArray(Data.results) && Data.results.length ? Data.results.map((item, i, Data) => (
                                            <tr className="border-bottom text-center" key={i}>
                                                ...
                                                <td className="border-right py-1 px-2"><span className="d-block fs-6 fw-bolder text-left text-dark px-2" >{item.PartyTitle} <br /> <small className='p-0 m-0'>{item.Address} </small></span></td>
