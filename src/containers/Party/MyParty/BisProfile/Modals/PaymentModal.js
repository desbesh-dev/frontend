...
import { PaymentRecipt } from "../../../../../actions/PartyAPI";
...
export const CreatePaymentModal = (props) => {
    ...
    const Receipt = async (e) => {
        ...
        if (isValid) {
            const result = await PaymentRecipt(props.item.id, payment, discount, paidAmount, due, IsBank, IsCheque, IsCard, IsOnline, bank, ACName, ACNumber, ChequeNo, TrxNo);
            ...
            if (result !== true) {
                ...
                props.setList([...props.list, toastProperties = {
                    id: 1,
                    title: result.Title,
                    description: "Party payment receipt failed. Please try after some moment.",
                    backgroundColor: '#f0ad4e',
                    icon: result.ico === 1 ? successIcon : result.ico === 2 ? infoIcon : result.ico === 3 ? warningIcon : result.ico === 4 ? errorIcon : null
                }])
            }
        ...
        >
            <Modal.Header className="py-2 border-bottom-0" closeButton>
                <p id="contained-modal-title-vcenter" className="fs-4 fw-bold text-dark text-center m-0 justify-content-center w-100">
                    Make Payment Receipt <br />
                    <small>Generating Payment Receipt Voucher</small>
                </p>
            </Modal.Header>
            <Modal.Body>
                <table className="table my-table table-borderless text-nowrap">
                    <tbody>
                        <tr className="border-top">
                            ...
                            <td>Items:</td>
                            <td className="fw-bolder text-right"><span>{props.item.ItemCount}</span></td>
                        </tr>
                        <tr>
                            <td>Order Date:</td>
                            <td><span className="fw-bold">{moment(props.item?.OrderDate).format("DD MMM YYYY") ?? "N/A"}</span></td>
                            <td>Order No:</td>
                        ...
                        <tr className="border-bottom">
                            <td>Payment:</td>
                            <td colSpan="3"><span className="fw-bold">{getLabel(props.item?.Payment, PaymentTerms) ?? "N/A"}</span></td>
                            <td>Paid:</td>
                            <td className="fw-bolder text-right"><span>{parseFloat(Paid).toLocaleString("en-BD", { minimumFractionDigits: 2 })}</span></td>
                        </tr>
                        <tr className="border-bottom">
                            <td>Salesman:</td>
                            <td colSpan="3"><span className="fw-bold">{props.item.Operator}</span></td>
                ...
                <div className="d-flex justify-content-around align-items-center">
                    <button className="btn btn-outline-success rounded-circle text-center fw-bolder fs-2" onClick={(e) => Receipt(e)}><i className="fad fa-check"></i></button>
