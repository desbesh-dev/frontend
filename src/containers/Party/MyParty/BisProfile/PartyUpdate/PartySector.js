...
export const PartySector = (props) => {
    ...
    const { Address, Contact, SCMoney, Limit, Balance, Target, Currency, FarmReg, BlankCheque, Agreement, Status } = formData;
    const onChange = e => setFormData({ ...formData, [e.target.name]: e.target.value });

    const SendContract = async () => {
        const result = await UpdatePartySector(props.item.id, Address, Contact, SCMoney, Limit, Balance, Target, Currency, FarmReg, BlankCheque, Agreement, Status);
        if (result !== true) {
            if (result.error) {
                const updatedState = {};
