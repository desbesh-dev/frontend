...
import { FetchPartyAgedInvoices, FetchPrintInvoice } from '../../../../../actions/PartyAPI';
...
const AgedInvoices = ({ user, PartyID, Title, Name, Contact, Address, Limit }) => {
    ...
    const AgedInvoiceList = async () => {
        ...
        var result = await FetchPartyAgedInvoices(PartyID, date_from, date_to);
        if (result !== true)
            setData(result.data);
        ...
        <div className="col-md-12 justify-content-center align-items-center px-2">
            <div className={`d-flex justify-content-between bg-white py-1 px-0`}>
                ...
                <div className="d-flex justify-content-end align-items-center bg-white p-0 w-75">
                    ...
                    <button className="btn fs-3 px-2 ml-2 py-0 text-dark border-left" onClick={(e) => AgedInvoiceListPDF(e, Data, { Title, Name, Contact, Address, Limit }, user, DateFrom, DateTo)}><i className="fad fa-file-pdf"></i></button>
                </div>
            </div>
