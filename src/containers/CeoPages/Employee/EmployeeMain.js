...
import { ApproveData, BankUpdate, Designation, LoadBanks, LoadProfile, RemoveBank, SaveBank, UpdateImage, getBranch, Designation_CEO } from '../../../actions/APIHandler';
...
const EmployeeMain = ({ UserID, display, user, list, setList, scale, sub_scale }) => {
    ...
    const FetchUser = async (UserID) => {
        ...
        if (User_Data !== true) {
            ...
            setFormData({
                ...
                BranchName: User_Data.UserInfo.BranchID.Name,
                is_subscriber: User_Data.UserInfo.is_subscriber,
                is_staff: User_Data.UserInfo.Role.is_staff,
                is_block: User_Data.UserInfo.Role.is_block,
                is_active: User_Data.UserInfo.Role.is_active,
    ...
    const UpdateUser = () => {
        if (Data)
            history.push('/update_user', { UserData: Data });
    }
        ...
        <>
            <div className="row">
                <div className="img_container">
                    <img src={ImagePrv ? ImagePrv : Data ? process.env.REACT_APP_API_URL + Image : ''} className="img rounded mb-0 mx-auto d-table" width="100" height="105" alt="avatar" />
                    {/* <img src={Data ? "http://127.0.0.1:8000" + Image : ''} className="img rounded mb-0 mx-auto d-table" width="100" height="105" alt="avatar" /> */}
            ...
            <div className="header mb-1">
                ...
                <p className="fw-bold text-success d-flex justify-content-center m-0">
                    {

                        is_subscriber ? 'Subscriber' :
                            is_staff ? getStaffLabel(is_staff, Designation_CEO) :
                                is_admin ? 'Admin' : null
                    } of {BranchName ? BranchName + " Branch" : null}
                </p>
                <p className="fw-bold text-dark d-flex justify-content-center m-0">{MobileNo ? MobileNo : null}{ContactNo ? ", " + ContactNo : null} {email ? ", " + email : null}</p>
            </div>

            <div className="header d-flex justify-content-center bg-white shadow-sm mb-2">
                <p className='fw-bold fs-4 text-success py-1 px-2 m-0' title="Activity" type='button' onClick={() => { setActivity(true); setProfile(false); }}>Activity</p>
                <div className="cs_outer bg-light m-0 my-auto" style={{ height: "30px" }}>
                    {/* <div className="cs_inner"></div> */}
                </div>
                <p className='fw-bold fs-4 text-success py-1 px-2 m-0' title="Profile" type='button' onClick={() => { setActivity(false); setProfile(true); }}>Profile</p>
                {
                    scale === 6 || (scale === 3 && (sub_scale === 9 || sub_scale === 10)) ?
                        <Fragment>
                            <div className="cs_outer bg-light m-0 my-auto" style={{ height: "30px" }} />
                            <p className='fw-bold fs-4 text-success py-1 px-2 m-0' title="Profile" type='button' onClick={() => UpdateUser()}>Update</p>
                        </Fragment>
                        : null
                }
            </div>
            ...
            {Profile ? EmployeeProfile(UserID, display, user, list, setList) : EmployeeActivity(UserID, display, user)}
        </>
        //     </div >
        // </div >
    );
...
export default connect(mapStateToProps, { logout })(EmployeeMain);