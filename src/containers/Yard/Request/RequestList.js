...
import { FetchPrintRequest } from '../../../actions/YardAPI';
import '../../../hocs/react-select/dist/react-select.css';
import { Pagination } from './RequestListPagination';
...
const RequestList = ({ user, list, setList, no }) => {
    ...
    const fetchData = useCallback(async (e) => {
        ...
        setLoading(true);
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/request_list/`, {
            headers: {
    ...
    const PrintPDF = async (e, item) => {
        ...
        // }
        // else {
        var result = await FetchPrintRequest(item.id);
        if (result !== true)
            RequestPrint(e, result, false)
        dispatch({ type: DISPLAY_OVERLAY, payload: false });
        // }
    }
            ...
            <div className="col-lg-12 h-100 px-0">
                ...
                <div className="row d-flex bg-white mx-auto my-2 py-1 m-0">
                    ...
                    <div className="d-flex justify-content-between align-items-center bg-white p-0">
                        <p className='fs-3 bg-white fw-bolder text-dark text-nowrap my-0 px-2'>REQUEST LISTS</p>
                        <div className="d-flex justify-content-around mx-2 w-100">
                            <p className='text-dark fs-4 fw-bold m-0 border border-light px-2' style={{ borderRadius: "15px" }}>Request: {parseFloat(Data?.count).toLocaleString("en-GB", { minimumFractionDigits: 0 })}</p>
                            ...
                            <table className={`table table-hover table-borderless bg-white text-nowrap`}>
                                <thead className='bg-white'>
                                    <tr className="text-center">
                                        ...
                                        <th className="py-1 border-right"><span>Deliver To</span></th>
                                        <th className="py-1 border-right"><span>Request No</span></th>
                                        ...
                                        Data.results.map((item, i) => {
                                            ...
                                            return (
                                                <tr className="border-bottom text-center fw-bold" key={i}>
                                                    ...
                                                    <td className="py-0 px-1 border-right text-center">{item.RequestNo}</td>
...
export default connect(mapStateToProps, { logout })(RequestList);