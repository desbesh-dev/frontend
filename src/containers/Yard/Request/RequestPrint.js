...
export const RequestPrint = async (e, item, status) => {
    var JsBarcode = require('jsbarcode');
    const name = item.ReqToSister;
    var cmpAd = item.ReqToAdd
    ...
    const contact = [
        item.ReqAdd && `Address: ${item.ReqAdd}`,
        item.ReqPhone && `Phone: ${item.ReqPhone}`,
        item.ReqContact && `Contact: ${item.ReqContact}`,
        item.ReqEmail && `Email: ${item.ReqEmail}`
    ].filter(Boolean).join(", ") || "";

    var add = contact.substring(0, 200) + (contact.length < 200 ? "" : "...");
    add = doc.splitTextToSize(add, 450);
    var site_top = doc.getTextDimensions(add).h;
    doc.setFontSize(10).setTextColor(51, 51, 51).setFont("courier", 'normal').text(add, marginLeft + 94, marginTop + 38)
    ...
    doc.setFontSize(11).setTextColor(0, 0, 0).setFont("courier", 'bold').text("Site: " + item.ReqToSector + " (" + item.ReqToShortCode + ")", marginLeft + 94, marginTop + site_top + 38)

    doc.setDrawColor(220, 220, 220);
    doc.setLineWidth(2);
    doc.line(marginLeft, 91, 570, 91);
    ...
    doc.setFillColor(255, 255, 255).rect(x - 98, 88, width, height, 'F');
    doc.setFontSize(16).setTextColor(0, 0, 0).setFont('helvetica', 'bold').text("PRODUCT REQUISITION", 480, 95, { align: "center" });
    ...
    // By
    doc.setFontSize(12).setTextColor(0, 0, 0).setFont("courier", 'bold').text("REQUEST BY: ", marginLeft, 115)
    var req_by_title = doc.splitTextToSize(item.ReqSister + " (" + item.ReqShortCode + ")", 250);
    doc.setFontSize(11).setTextColor(0, 0, 0).setFont("courier", 'normal').text(req_by_title, marginLeft, 127)
    doc.setFontSize(11).setTextColor(0, 0, 0).setFont("courier", 'normal').text("Contact: " + item.ReqPhone, marginLeft, 139)

    var req_by_add = item.ReqForAdd.substring(0, 70) + (item.ReqForAdd.length < 70 ? "" : "...");
    req_by_add = doc.splitTextToSize(req_by_add, 190);
    var ht = doc.getTextDimensions(req_by_add).h;
    doc.setFontSize(11).setTextColor(0, 0, 0).setFont("courier", 'normal').text(req_by_add, marginLeft, 150)
    doc.setFontSize(11).setTextColor(0, 0, 0).setFont("courier", 'bold').text("Payment: " + getPaymentShort(item.Payment, PaymentTerms), marginLeft, 139 + ht + 11)


    // For
    doc.setFontSize(12).setTextColor(0, 0, 0).setFont("courier", 'bold').text("REQUEST FOR: ", 350, 115)
    var party_title = doc.splitTextToSize(item.ReqForSister + " (" + item.ReqForShortCode + ")", 250);
    doc.setFontSize(11).setTextColor(0, 0, 0).setFont("courier", 'normal').text(party_title, 350, 127)
    doc.setFontSize(11).setTextColor(0, 0, 0).setFont("courier", 'normal').text("Contact: " + item.ReqForPhone, 350, 139)

    var party_address = item.ReqForAdd.substring(0, 70) + (item.ReqForAdd.length < 70 ? "" : "...");
    party_address = doc.splitTextToSize(party_address, 190);
    var ht = doc.getTextDimensions(party_address).h;
    doc.setFontSize(11).setTextColor(0, 0, 0).setFont("courier", 'normal').text(party_address, 350, 150)
    doc.setFontSize(11).setTextColor(0, 0, 0).setFont("courier", 'bold').text("Requisation No: " + item.RequestNo, 350, 139 + ht + 11)
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        if (i === pageCount) {
            // Prepare by
            doc.setFontSize(11).setTextColor(0, 0, 0).setFont("helvetica", 'bold').text(item.Operator, doc.internal.pageSize.getWidth() / 10, pageHeight - 72, { align: "left" })
            ...
            doc.setFontSize(11).setTextColor(0, 0, 0).setFont("helvetica", 'normal').text("Prepared By", doc.internal.pageSize.getWidth() / 10, pageHeight - 60, { align: "left" })
    ...
    const fileName = "Request No-" + item.RequestNo + " Date-" + moment(item.Date).format("DD MMM YYYY")
