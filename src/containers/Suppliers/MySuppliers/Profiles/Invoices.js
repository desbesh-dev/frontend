...
import { FetchPurchaseInvoice, FetchSupplierInvoices } from '../../../../actions/SuppliersAPI';
...
const Invoices = ({ user, PartyID, SupplierID, id, list, setList }) => {
    ...
    const InvoiceList = async () => {
        ...
        var result = await FetchSupplierInvoices(SupplierID, date_from, date_to);
    ...
    const FilterInvoice = Data.length && Data?.filter(({ Payment, InvoiceNo, PurchaseNo }) =>
        (!PayTypeFilter || Payment === PayTypeFilter.value) &&
        (!SearchKey || InvoiceNo === SearchKey.value || PurchaseNo === SearchKey.value)
    ).map(({ id, InvDate, RcvDate, OrderNo, PurchaseNo, InvoiceNo, Count, VatRate, Vat, Discount, GrandTotal, PaidAmount, Due, Payment, Receiver, Status, PaymentStatus }) => ({
        id, InvDate, RcvDate, OrderNo, PurchaseNo, InvoiceNo, Count, VatRate, Vat, Discount, GrandTotal, PaidAmount, Due, Payment, Receiver, Status, PaymentStatus
    }));
    ...
    return (
        <div className="col-md-12 justify-content-center align-items-center px-2">
            <div className={`d-flex justify-content-between bg-white my-1 p-0`}>
                <p className='display-6 bg-white text-nowrap fw-bolder m-0 px-1'>Purchase Invoices</p>
