...
export const AgedInvoiceListPDF = async (e, item, Data, user, date_from, date_to) => {
    ...
    const SupplierTitle = Data.SupplierTitle;
    // const PartyName = Data.Name
    const ContactSupplier = Data.Contact
    ...
    doc.setFontSize(15).setTextColor(0, 0, 0).setFont(undefined, 'bold').text(SupplierTitle, doc.internal.pageSize.getWidth() / 2, 40, { align: "center" })
    doc.setFontSize(10).setTextColor(54, 69, 79).setFont(undefined, 'normal').text(ContactSupplier, doc.internal.pageSize.getWidth() / 2, 52, { align: "center" })
    var splitAdd = doc.splitTextToSize(subscriberAd, 260);
    doc.setFontSize(9).setTextColor(85, 87, 104).setFont(undefined, 'normal').text(splitAdd, doc.internal.pageSize.getWidth() / 2, 64, { align: "center" })
    ...
    doc.setFontSize(11).setTextColor(0, 0, 0).setFont(undefined, 'bolditalic').text(dateRangeText, doc.internal.pageSize.getWidth() / 2, 90, { align: "center" });
    ...
    item.forEach(({ PaidAmount, Due, GrandTotal, InvDate }) => {
    ...
    const headers = [["S/N", "Date", "Received", "Invoice No", "Days", "Payment Status", "Payment Term", "Paid", "Due", "1 - 30 days", "31 - 61 days", "62 - 92 days", "More..."]];
    ...
    const fileName = `Aged Invoices (${SupplierTitle}) ${moment(item.InvDate).format("DD MMM YYYY")}`
