...
import { FetchProductDetails, FetchPurchaseNo, PaymentTerms } from '../../../actions/InventoryAPI';
...
const PurchaseProduct = ({ display, SupplierID, CompanyID, BranchID, user, list, setList }) => {
    ...
    const SavePurchase = async () => {
        ...
        var data = moment(InvDate).format('YYYY-MM-DD')
        var rcv_today = moment(today).format('YYYY-MM-DD')
        PurchaseData = PurchaseData.map((item, index) => ({ ...item, SLNo: index + 1 }));
        dispatch({ type: DISPLAY_OVERLAY, payload: true });
        var result = await Purchase(user.Collocation.id, '', SupplierID, PurchaseNo, OrderNo, OrderBy, user.id, InvoiceNo, DocketNo, data, rcv_today, user.id, Payment.value, Vat, parseFloat(VatTotal).toFixed(2), Discount, parseFloat(GrandTotal).toFixed(2), Paid, parseFloat(Due).toFixed(2), Count, PurchaseData);
        ...
        <div className="row h-100 m-0 d-flex justify-content-center">
            <div className="header mb-4">
                <nav aria-label="breadcrumb">
                    ...
                    <p className="display-6 d-flex justify-content-center m-0">Product Purchase Voucher</p>
                </nav>
            </div>
