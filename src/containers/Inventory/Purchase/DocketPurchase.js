...
import { FetchProductDetails, PaymentTerms } from '../../../actions/InventoryAPI';
import { FetchPurchaseInvoice, PurchaseUpdate } from '../../../actions/SuppliersAPI';
...
const DocketPurchase = ({ PurchaseID, user, list, setList }) => {
    ...
    async function LoadSuppliers() {
        ...
        if (result !== true) {
            ...
            setInvDate(new window.Date(result.InvDate))
            setRcvDate(new window.Date(result.RcvDate))
            setInvoiceNo(result.InvoiceNo)
            setDocketNo(result.DocketNo)
            setOrderNo(result.OrderNo)
            setOrderBy(result.OrderBy)
            setReceivedBy(result.Receiver)
            setReceiver(result.ReceiverID)
            setPayment({ value: result.Payment, label: getLabel(result.Payment, PaymentTerms) })
            setPrvCount(result.Count)

            localStorage.setItem("supplierData", JSON.stringify(result));
            localStorage.setItem("supplierDataTimestamp", today.getTime());
            LoadSupplierProduct(Supplier = { Status: 1 }, result.SupplierID);
        }
        dispatch({ type: DISPLAY_OVERLAY, payload: false });
    }
    ...
    const LoadSupplierProduct = async (Status, SupplierID) => {
        dispatch({ type: DISPLAY_OVERLAY, payload: true });
        if (Status) {
            var ProductItems = await SupplierProductDropdown(SupplierID);
            if (ProductItems !== true)
                setMyProList(ProductItems.data);
        }
        else {
            var ProductItems = await SupplierProductAll(SupplierID);
            if (ProductItems !== true)
                setMyProList(ProductItems.data);
        }
        dispatch({ type: DISPLAY_OVERLAY, payload: false });
    ...
    const SavePurchase = async () => {
        ...
        var data = moment(RcvDate).format('YYYY-MM-DD')
        PurchaseData = PurchaseData.map((item, index) => ({ ...item, SLNo: index + 1 + prvCount }));
        dispatch({ type: DISPLAY_OVERLAY, payload: true });
        var result = await PurchaseUpdate(PurchaseID, InvoiceNo, DocketNo, Payment, data, Vat, VatTotal, Discount, GrandTotal, Paid, Due, Count, PurchaseData);
    ...
    const validateForm = () => {
        ...
        if (!PurchaseID || (PurchaseID && PurchaseID.trim() === "")) {
            errors.SupplierID = "Purchase ID can't identified";
        }
        ...
        <div className="row h-100 m-0 d-flex justify-content-center">
            <div className="header mb-4">
                <nav aria-label="breadcrumb">
                    ...
                    <p className="display-6 d-flex justify-content-center m-0">Docket Entry</p>
                </nav>
            </div>
                                    ...
                                    <tbody>
                                        {
                                            PurchaseData.slice().reverse().map((item, i) => (
                                                <tr className="border-bottom text-center" key={i}>
                                                    <td className="p-0 border-right"><span className="d-block fw-bold">{i + prvCount + 1}</span> </td>
...
const mapStateToProps = (state, props) => ({
    ...
    PurchaseID: props.match.params.purs_id,
    user: state.auth.user
});

export default connect(mapStateToProps, { logout })(DocketPurchase);