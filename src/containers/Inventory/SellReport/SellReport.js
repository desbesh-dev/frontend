...
import { Receipt } from '../../Trading/CounterReceipt';
import { InvoicePrint } from '../../Trading/InvoicePrint';
import { Pagination } from './SellReportPagination';

let today = new Date();
const SellReport = ({ user, list, setList, no }) => {
    ...
    const fetchData = useCallback(async (e) => {
        ...
        setData(res.data);
        setLoading(false);
    }, [Date, currentPage, itemsPerPage, SectorFilter, PayTypeFilter, SearchKey, DateTo, CounterFilter]);
                ...
                <div className="row d-flex bg-white mx-auto my-2 py-1 m-0">
                    ...
                    <div className="d-flex justify-content-between align-items-center bg-white p-0">
                        ...
                        <div className="d-flex justify-content-around mx-2 w-100">
                            ...
                            <p className='text-dark fs-4 fw-bold m-0 border border-light px-2' style={{ borderRadius: "15px" }}>Sale: {parseFloat(Data?.Sale).toLocaleString("en-GB", { minimumFractionDigits: 2 })}</p>
                    ...
                    <div className="d-flex justify-content-around align-items-center bg-white p-0">
                        ...
                        <div className="d-flex justify-content-end mx-2 w-25">
                            <Select
                                ...
                                defaultValue={{ label: "Select Dept", value: 0 }}
                                name="Type"
                                placeholder={"Counter Filter"}
                                styles={CScolourStyles}
                                value={CounterFilter}
                                onChange={(e) => { setCounterFilter(e); setCurrentPage(1) }}
                                required
                                id="Type"
                                isClearable={true}
                                isSearchable={true}
                        ...
                        <div className="d-flex justify-content-end mx-2 w-25">
                            <Select
                                ...
                                styles={CScolourStyles}
                                value={PayTypeFilter}
                                onChange={(e) => { setPayTypeFilter(e); setCurrentPage(1) }}
                                required
                                id="PayTypeFilter"
                                isClearable={true}
                                isSearchable={true}
                            ...
                            <table className={`table table-hover table-borderless text-nowrap`}>
                                <thead>
                                    <tr className="text-center">
                                        ...
                                        <th className="border-right p-1"> <span className="fs-6 fw-bolder text-primary text-uppercase p-0">Due</span>/<span className="fs-6 fw-bolder text-danger text-uppercase p-0">CND</span></th>
                                        ...
                                        <th className="border-right p-1"> <span className="fs-6 fw-bolder text-dark text-uppercase p-0">C/R</span></th>
                                ...
                                <tbody>
                                    {Data.results.map((item, i) => {
                                        const { Index, Date, InvoiceNo, ItemCount, Bank = 0.00, Cash = 0.00, Discount = 0.00, GrandTotal = 0.00, PaidAmount = 0.00, RefundAmount = 0.00, Due = 0.00, PartyName = "Walk-in", Payment, CounterNo, Operator, SectorTitle, Sister, Count, Return } = item
                                        return (
                                            <tr className="border-bottom text-center" key={i}>
                                                ...
                                                <td className="border-right p-1"><span className="d-block fs-6 fw-bold text-center text-dark p-0">{CounterNo}</span></td>
