...
import { LoadPayment, DeleteDR } from '../../../actions/ContractAPI';
...
const Payment = ({ UserID, BisID, BatchID, user, list, setList }) => {
    ...
    const GetPayment = async () => {
        if (user !== null) {
            ...
            var result = await LoadPayment(BatchID);
                                ...
                                <table className={`table table-hover`} style={{ maxHeight: "70%" }}>
                                    <thead>
                                        <tr className="text-center">
                                            <th className="p-1 border-0" colspan="14">
                                                <div className="d-flex">
                                                    ...
                                                    <p className="mr-auto p-0 fs-4 fw-bolder mb-0 text-center text-uppercase">Payment History</p>
                                        ...
                                        <tr className="text-center" style={{ borderBottom: "3px solid #DEE2E6" }}>
                                            ...
                                            <th className="border-right py-1 align-middle"><span className="fs-6 fw-bolder text-dark text-uppercase">Pay To</span></th>
                                            ...
                                            <th className="border-right py-1 align-middle"><span className="fs-6 fw-bolder text-dark text-uppercase">Plan</span></th>
                                            <th className="border-right py-1 align-middle"><span className="fs-6 fw-bolder text-dark text-uppercase">Status</span></th>
                                            <th className="border-top py-1 align-middle"><span className="fs-6 fw-bolder text-dark text-uppercase">Remark</span></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {Data.map((item, i) => (
                                            <>
                                                <tr className="border-bottom text-center" key={i}>
                                                    ...
                                                    <td className="border-right p-0"><span className="d-block fs-6 fw-bold text-center text-dark px-2">{moment(item.VoucherNo.Dte).format("DD MMM YYYY")}</span></td>
                                                    <td className="border-right p-0"><span className="d-block fs-6 fw-bold text-center text-dark px-2">{item.BatchID.BusinessID.Title}</span> </td>
                                                    <td className="border-right p-0"><span className="d-block fs-6 fw-bold text-right text-dark px-2">{parseFloat(item.CR).toLocaleString("en-BD", { minimumFractionDigits: 2 }) + " ৳"}</span> </td>
                                                    <td className="border-right p-0"><span className="d-block fs-6 fw-bold text-right text-dark px-2">{item.VoucherNo.PaymentMethod}</span> </td>
