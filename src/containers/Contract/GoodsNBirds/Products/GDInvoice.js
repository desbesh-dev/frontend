...
import { GetGodown, GodownInvoice } from '../../../../actions/ContractAPI';
...
let today = new Date();
const GDInvoice = ({ display, GID, CompanyID, BranchID, user, list, setList }) => {
    ...
    const LoadInvoiceNo = async () => {
        ...
        var result = await FetchInvoiceNo('GI');
    ...
    const SaveInvoice = async () => {
        let ProductSent_ID = Godown.Accounts.GodownAC.id
        let ProductSent_Code = Godown.Accounts.GodownAC.COA_Code

        dispatch({ type: DISPLAY_OVERLAY, payload: true });
        var data = moment(Date).format('YYYY-MM-DD')
        var result = await GodownInvoice(Godown.id, InvoiceNo, data, Vat, (getTotal() * Vat) / 100, Discount, Shipment, TotalPrice(), Count, SellData, ProductSent_ID, ProductSent_Code);
        if (result !== true) {
            if (result.error) {
                const updatedState = {};
    ...
    const BarcodeGen = (e) => {
        let Bar = CompanyID + BranchID + GID + (InvoiceNo ? InvoiceNo : "") + ItemCode + e.target.value;
        setFormData({
            ...formData,
    ...
    return (
        <div className="row h-100 m-0 d-flex justify-content-center">
            <div className="header mb-4">
                <p className="display-6 d-flex justify-content-center m-0">Product Dispatch To Godown</p>
                <nav aria-label="breadcrumb">
                    <ol className="breadcrumb d-flex justify-content-center">
                        <li className="breadcrumb-item"><Link to="/goods_n_bird">Home</Link></li>
                        <li className="breadcrumb-item"><Link to={`/gd_invoice/${GID}`}>Send Product</Link></li>
                    </ol>
                </nav>
            </div>
                            ...
                            Godown ?
                                <Fragment>
                                    <div className="row g-0">
                                        <p className="fs-6 text-success text-center fw-bold m-0">Godown Info</p>
                                        <div className="col-md-12">
                                            <div className="card-body">
                                                <p className="display-6 fw-bold m-0">{Godown.id + ". " + Godown.Title}</p>
                                                <p className="fs-6 m-0">{Godown.BranchName} Branch, {Godown.Type === 1 ? "Live Birds" : Godown.Type === 2 ? "Proccesed Birds" : "N/A"}</p>
                                                <p className="fs-6 m-0">{Godown.RepName + ", " + Godown.RepContact}</p>
                                            </div>
                                        </div>
                                    </div>
                                    <small className="text-muted text-left fw-bold">* Account- {Godown.Accounts.GodownAC.COA_Title}</small>
                                </Fragment>
                                :
                                null
                        }
...
export default connect(mapStateToProps, { logout })(GDInvoice);