...
import { FetchFCRRank } from '../../../actions/BoardAPI';

const FCR_Rank = ({ CompanyID, BranchID, SupplierID, user, list, setList }) => {
    ...
    const LoadFCRRank = async () => {
        ...
        var result = await FetchFCRRank(Batch, Year);
    ...
    const FetchBatch = async (e) => {
        dispatch({ type: DISPLAY_OVERLAY, payload: true });
        var result = await FetchFCRRank(e.target.value | 0, Year | 'null');
        if (result.status === 200) {
            setData(result.data);
        }
        dispatch({ type: DISPLAY_OVERLAY, payload: false });
    ...
    const FetchYear = async (e) => {
        dispatch({ type: DISPLAY_OVERLAY, payload: true });
        var result = await FetchFCRRank(Batch | 0, e | 'null');
        if (result.status === 200) {
            setData(result.data);
            setYear(e);
        }
        ...
        <div className="position-relative h-100">
            <div className="container bg-white rounded">
                ...
                <div className='text-center mt-2'>
                    <p className='display-6 fw-bolder m-0 text-uppercase text-center text-muted'> FEED CONVERSION RANK <span className="text-center fw-bolder">{Year ? "- " + Year : null}</span></p>
                </div>
                                ...
                                <table className={`table table-hover table-borderless table-responsive d-table`}>
                                    <thead>
                                        <tr className="text-center text-uppercase" style={{ borderBottom: "3px solid #DEE2E6" }}>
                                            ...
                                            <th className="p-1 border-right"><span className="d-block text-center fw-bolder">AVG FCR</span></th>
...

                                        {
                                            Data.map((item, i) => (
                                                <tr className="border-bottom text-center" key={1}>
                                                    ...
                                                    <td className="p-0 border-right"><span className="d-block fw-bold text-center px-1">{parseFloat(item.AVG_FCR).toFixed(2)}</span> </td>
...
export default connect(mapStateToProps, { logout })(FCR_Rank);