...
import { InvoicePrint } from '../InvoicePrint';
import { OrderPrint } from '../SaleOrder/OrderPrint';
import { DeleteModal } from "./Modals/ModalForm.js";
...
const QuotePreview = ({ user, list, setList, QuoteID }) => {
    ...
    const GetInvoiceData = async () => {
        ...
        setItem(false);
        var result = await FetchPrintQuote(QuoteID);
        if (result !== true)
            setData(result)
    }
    ...
    const getTotal = () => {
        let TotalPrice = 0;
        const price = Data.QuoteMapData.map(row => row.Qty * row.Rate);
        if (price.length > 0) {
    ...
    function calculateTotals(dataType) {
        let QuoteMapData;
        if (dataType === "QuoteMapData") {
            QuoteMapData = Data?.QuoteMapData || [];
    ...
    const { QuantityTotal = "0", WeightTotal = "0.000" } = calculateTotals("QuoteMapData");
        ...
        <div className="container bg-white rounded px-2" style={{ position: "relative" }}>
            <div className="d-flex justify-content-between my-0 bg-white border-bottom py-2">
                <div className='my-auto text-center'>
                    <p className='display-6 fw-bolder m-0 text-uppercase text-left'><button className=" btn fs-3 fad fa-chevron-left px-2 text-dark text-decoration-none" onClick={() => history.goBack()}></button> Quotation Preview</p>
                </div>
                ...
                {Data ?
                    <div className='text-right'>
                        <button title="Delete Quote" className="btn fs-3 px-2 py-0 fad fa-trash-alt text-dark" id="view" onClick={() => { setDeleteData(Data); setDeleteModalShow(true) }} />
                        <button title="Edit Quote" className="btn fs-3 px-2 py-0 fad fa-edit text-dark" id="view" />
                    </div> : null}
            </div>
                ...
                <div className="col-md-12">
                            ...
                            Data ?
                                <div className='my-auto text-left'>
                                    ...
                                    <div className="card-body p-0">
                                        ...
                                        <p className='fw-bold m-0'>{`${Data?.SalesMan}`}
                        ...
                        <div className='text-right'>
                            {
                                Data ?
                                    <div className="card-body p-0">
                                        <p className="fs-4 fw-bold m-0">{Data?.Name}</p>
                                        <small className="text-muted p-0">{Data?.Address}</small>
                    ...
                    <div className="row pt-2">
                        <div className="col-md-12 text-center">
                            <p className="fs-3 fw-bolder m-0">{`QUOTE# ${Data?.QuoteNo}`}</p>
                        </div>
                    </div> <br />
                    {Array.isArray(Data.QuoteMapData) && Data.QuoteMapData.length ?
...
export default connect(mapStateToProps, { logout })(QuotePreview);