...
import { DeleteQuote } from '../../../actions/InventoryAPI';
...
let today = new Date();
const QuoteList = ({ user, list, setList, no }) => {
    ...
    const fetchData = useCallback(async (e) => {
        ...
        setLoading(true);
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/quote_list/`, {
            headers: {
            ...
            <div className="col-lg-12 h-100 px-0">
                ...
                <div className="row d-flex bg-white mx-auto my-2 py-1 m-0">
                    ...
                    <div className="d-flex justify-content-between align-items-center bg-white p-0">
                        <p className='fs-3 bg-white fw-bolder text-dark text-nowrap my-0 px-2'>QUOTATION LISTS</p>
                        <div className="d-flex justify-content-around mx-2 w-100">
                            <p className='text-dark fs-4 fw-bold m-0 border border-light px-2' style={{ borderRadius: "15px" }}>Quote: {parseFloat(Data?.count).toLocaleString("en-GB", { minimumFractionDigits: 0 })}</p>
                            ...
                            <table className={`table table-hover table-borderless text-nowrap`}>
                                <thead>
                                    <tr className="text-center">
                                        ...
                                        <th className="border-right p-1"> <span className="fs-6 fw-bolder text-dark text-uppercase p-0">Name</span></th>
                                        <th className="border-right p-1"> <span className="fs-6 fw-bolder text-dark text-uppercase p-0">Quote No</span></th>
                                ...
                                <tbody>
                                    {Data.results.map((item, i) => {
                                        const { Index, Date, Name, PartyID, QuoteNo, ItemCount, Qty = 0.00, Weight = 0.000, Vat = 0.00, Discount = 0.00, GrandTotal = 0.00, Operator, SectorTitle, Sister, Count } = item
                                        return (
                                            <tr className="border-bottom text-center" key={i}>
                                                ...
                                                <td className="border-right p-1 d-block fs-6 text-left text-dark p-0">{Name}<span className="fw-bolder text-warning"> {!PartyID ? "(Walk-in)" : ""}</span></td>

                                                <td className="border-right p-1"><span className={`d-block fs-6 text-center p-0`}>{QuoteNo}</span></td>
                                                ...
                                                {/* <td className="border-right p-1"><span className="d-block fs-6 fw-bold text-right text-dark p-0">{parseFloat(Weight).toLocaleString("en", { minimumFractionDigits: 3 })}</span></td> */}
                                                <td className="border-right p-1"><span className="d-block fs-6 fw-bold text-right text-dark p-0">{parseFloat(Vat).toLocaleString("en", { minimumFractionDigits: 2 })}</span></td>
                                                ...
                                                <td rowSpan={parseInt(Count)} className="p-1"><span className="d-block fs-6 fw-bold text-left text-dark p-0">
                                                    ...
                                                    <Link title="Edit" className="btn fs-5 px-2 py-0 fad fa-edit text-dark" to={`/quote_edit/${item.id}`} />
                                                    ...
                                                    <Link title="Show Quote" className="btn fs-5 px-2 py-0 fad fa-eye text-dark" id="view" to={`/quote_preview/${item.id}`} />
                                                </span>
                                                </td>
                                            </tr>
                                        )
...
export default connect(mapStateToProps, { logout })(QuoteList);