...
const QuoteOrder = ({ PartyID, QuoteID, user, list, setList }) => {
    ...
    const AddRow = (e) => {
        ...
        if (dataExistsIndex === -1) {
            ...
            OrderData[dataExistsIndex].Available = parseFloat(OrderData[dataExistsIndex].Available, 10) - parseFloat(formData.Qty, 10);
    ...
    const AutoFireData = async (e, FireData) => {
        if (!FireData.Qty || !FireData.Code) {
            ...
            if (dataExistsIndex === -1) {
                ...
                OrderData[dataExistsIndex].Qty = parseFloat(OrderData[dataExistsIndex].Qty, 10) + parseFloat(FireData.Qty, 10);
                ...
                OrderData[dataExistsIndex].Available = parseFloat(OrderData[dataExistsIndex].Available, 10) - parseFloat(FireData.Qty, 10);
                const updatedItem = OrderData.splice(dataExistsIndex, 1)[0];
                OrderData.push(updatedItem);
                setOrderData([...OrderData]);
            }
    ...
    const getTotal = () => {
        ...
        return OrderData.reduce((acc, { Qty, Rate }) => acc + Qty * Rate, 0.00);
    };

    const QuantityTotal = Array.isArray(OrderData) && OrderData.length ? OrderData.reduce((total, { Qty }) => total + parseInt(Qty, 10), 0) : 0;
    ...
    const SaveOrder = async (e) => {
        ...
        var result = await DeliveryOrder(user.Collocation.CounterID, PartyID, data, order_date, delivery_date, Vat, VatTotal, Discount, Shipment, Payment, GrandTotal, Paid, Due, RefundAmount, Count, OrderData);
        if (result !== true) {
            if (result.error) {
                const updatedState = {};
    ...
    const GetQuoteData = async () => {
        ...
        if (result !== true) {
            setOrderData(result.QuoteMapData);
        ...
        <div className="row d-flex m-0">
            <div className="d-flex py-2 m-0 justify-content-between align-items-center" style={{ zIndex: 1, backgroundColor: "#F4DCC1" }}>
                ...
                <Link className="d-flex justify-content-end align-items-center w-25" to={`/my_party/${PartyData?.PartyID?.id}/${PartyData?.id}`}>
                    <div className='row px-2'>
                        <p className={`fs-2 fw-bold m-0 text-right ${!PartyData && "text-muted"}`} style={{ fontFamily: "MyriadPro" }}>{PartyData?.PartyID?.Title || "N/A"}</p>
                    ...
                    <div className="d-flex justify-content-center align-items-center h-100">
                        ...
                        <form className="row justify-content-center mx-2 py-2 px-0 my-0 h-100">
                                ...
                                <div className="row justify-content-between p-0">
                                    ...
                                    <div className="col-md-7 justify-content-center align-items-center">
                                        ...
                                        <input
                                            ...
                                            disabled
                                        />
                                        {/* <BarcodeReader onScan={(e) => BarcodeHandler(MyProList, e)} /> */}
                            ...
                            <div className='justify-content-center align-self-end my-2 mx-0 p-0'>
                                <div className="row justify-content-between p-0 align-items-end">
                                    <div className="col-md-6">
                                        <p className="text-center fs-5 text-dark fw-bold m-0 border-bottom">QuoteOrder Date</p>
                                        <Datepicker
                                            selected={OrderDate}
...
export default connect(mapStateToProps, { logout })(QuoteOrder);