...
import { PaymentTerms, RtnSellItemDelete } from '../../../actions/InventoryAPI';
import { FetchPrintInvoice, FetchPrintOrder } from '../../../actions/PartyAPI';
import { Receipt } from '../CounterReceipt';
import { InvoicePrint } from '../InvoicePrint';
import { OrderPrint } from '../SaleOrder/OrderPrint';
import { CreateNote, DeleteModal, EditModal, InfoMessage } from "./Modals/ModalForm.js";
import { CashNotePrint, NotePrint } from './NotePrint';

const PreviewInvoice = ({ user, list, setList, InvoiceID }) => {
    ...
    const GetInvoiceData = async () => {
        ...
        setItem(false);
        var result = await FetchPrintInvoice(InvoiceID);
        if (result !== true) {
            setSellMapReturn(result?.SellMapReturn)
            setData(result)
        }
    }
    ...
    const RemoveRtnItem = async (e, id) => {
        ...
        dispatch({ type: DISPLAY_OVERLAY, payload: true });
        e.preventDefault();
        const result = await RtnSellItemDelete(id);
        if (result !== true) {
            if (result.user_error) {
                setList([...list, toastProperties = {
                    id: 1,
    ...
    const getTotal = () => {
        let TotalPrice = 0;
        const price = Data.SellMapData.map(row => row.Qty * row.Rate);
        if (price.length > 0) {
    ...
    function calculateTotals(dataType) {
        let SellMapData;
        if (dataType === "SellMapData") {
            SellMapData = Data?.SellMapData || [];
            return SellMapData.reduce((acc, { Qty, Weight }) => {
                acc.QuantityTotal += parseFloat(Qty, 10);
                acc.WeightTotal += parseFloat(Weight, 10);
                return acc;
            }, { QuantityTotal: 0, WeightTotal: 0.000 });
        } else if (dataType === "SellMapReturn") {
            SellMapData = SellMapReturn || [];
            return SellMapData.reduce((acc, { Qty, Weight, SubTotal }) => {
    ...
    const { QuantityTotal = "0", WeightTotal = "0.000" } = calculateTotals("SellMapData");
    const { RtnQty = "0", RtnWeight = "0.000", RtnSubTotal = "0.00" } = calculateTotals("SellMapReturn");

    const deleteRtn = (i) => {
        if (!Array.isArray(SellMapReturn) || !SellMapReturn) return;
        setSellMapReturn([...SellMapReturn.slice(0, i), ...SellMapReturn.slice(i + 1)]);
        setPaid(0.00);
    };
        ...
        <div className="container bg-white rounded px-2" style={{ position: "relative" }}>
            <div className="d-flex justify-content-between my-0 bg-white border-bottom py-2">
                <div className='my-auto text-center'>
                    <p className='display-6 fw-bolder m-0 text-uppercase text-left'><button className=" btn fs-3 fad fa-chevron-left px-2 text-dark text-decoration-none" onClick={() => history.goBack()}></button> Dispatch Invoice</p>
                </div>
                ...
                <div className="col-md-12">
                            ...
                            Data ?
                                <div className='my-auto text-left'>
                                    ...
                                    <div className="card-body p-0">
                                        ...
                                        <p className='fw-bold m-0'>{`${Data?.CounterMarry} (Counter No- ${Data?.CounterNo})`}
                                            <p className='fw-bolder m-0'>{`Invoice Date: ${moment(Data.Date).format("DD MMM YYYY")}`}</p>
                                        </p>
                                    </div>
                                </div>
                                : null
                        ...
                        <div className='text-right'>
                            {
                                Data ?
                                    <div className="card-body p-0">
                                        <p className="fs-4 fw-bold m-0">{Data?.PartyTitle}</p>
                                        <small className="text-muted p-0">{Data?.PartyContact}</small>
                    ...
                    <div className="row pt-2">
                        <div className="col-md-12 text-center">
                            <p className="fs-3 fw-bolder m-0">{`INVOICE# ${Data?.InvoiceNo}`}</p>
                        </div>
                    </div> <br />
                    {Array.isArray(Data.SellMapData) && Data.SellMapData.length ?
                            ...
                            <table className={`table table-hover table-borderless`}>
                                ...
                                <tbody>
                                    ...
                                    <tr className="text-center border-success bg-white">
                                        ...
                                        <td className="py-0 border-right text-right px-1">{parseFloat(Data?.Shipping || 0.00).toLocaleString("en", { minimumFractionDigits: 2 })}</td>
                                    </tr>
                                    <tr className="text-center border-success bg-white">
                                        <td className="py-0 px-1 border-right" colSpan="9"><span className="d-block text-right fw-bold">Total Price </span> </td>
                                        <td className="py-0 border-right px-1"><span className="d-block fw-bold text-right">{parseFloat(grand_tot || 0.00).toLocaleString("en", { minimumFractionDigits: 2 })}</span> </td>
                                    </tr>

                                    {
                                        Array.isArray(SellMapReturn) && SellMapReturn.length ?
                                            <>
                                                <tr className="border-top py-2" style={{ borderBottom: "3px solid #DEE2E6" }}>
                                                    <td className="py-1 d-table-cell" style={{ display: "table-cell", verticalAlign: "bottom" }}><span className="d-block fw-bolder px-1">{moment(Data.Note?.Date).format("DD MMM YY") || "N/A"}</span></td>
                                                    ...
                                                    <td colSpan={4} className="py-1 d-table-cell" style={{ display: "table-cell", verticalAlign: "bottom" }}>
                                                        <span className="d-block fw-bold px-1">
                                                            {Data.PartyID ? (Data.Note ? `Credit note has been issued. No# ${Data.Note?.NoteNo}` : "Credit note has not been issued yet") : (Data.Note ? `Cash refund has been issued. No# ${Data.Note?.NoteNo}` : "Cash refund has not been issued yet")}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td className="p-0 text-center d-table-cell" style={{ display: "table-cell", verticalAlign: "bottom" }}>
                                                        {
                                                            Data.Note ?
                                                                Data.PartyID ?
                                                                    <button className="btn fs-4 text-danger text-center" title="Credit Note" onClick={(e) => NotePrint(e, Data, false)}>
                                                                        <i className="fad fa-print"></i>
                                                                    </button>
                                                                    :
                                                                    <button className="btn fs-4 text-danger text-center" title="Credit Note" onClick={(e) => CashNotePrint(e, Data, false)}>
                                                                        <i className="fad fa-print"></i>
                                                                    </button>
                                                                :
                                                                <button className="btn fs-4 text-danger text-center" title="Credit Note" onClick={() => { setNoteItem({ PartyID: Data.PartyID ? Data?.PartyID : '', Name: Data.PartyID ? Data?.PartyTitle : "Walk-in", Amount: RtnSubTotal, NoteType: Data.PartyID ? 2 : 1, NoteName: Data.PartyID ? "Credit Note" : "Cash Refund", InvoiceNo: Data?.InvoiceNo, InvoiceID: Data?.id, Count: Array.isArray(SellMapReturn) && SellMapReturn.length }); setNoteModalShow(true) }}>
                                                                    <i className="fad fa-file-edit"></i>
                                                                </button>
                                                        }
                                                    </td>
                                                </tr>
                                                {SellMapReturn.map((item, i) => (
                                                    <tr className="border-bottom text-center" key={i}>
                                                        <td className="p-0 border-right"><span className="d-block fw-bold px-1">{item.SLNo}</span></td>
                                                        <td className="p-0 border-right"><span className="d-block fw-bold px-1">{item.Code}</span></td>
                                    ...
                                    <td colSpan={11} className="p-1 text-center border border-light mt-3">
                                        <span className="d-inline-block text-right fw-bold mr-3 p-0">Item: <span className="d-inline-block text-left fw-bolder mr-3 p-0">{Array.isArray(Data.SellMapData) && Data.SellMapData.length}</span></span>
                                        ...
                                        {Array.isArray(SellMapReturn) && SellMapReturn.length ?
                                            <Fragment>
                                                <span className="d-inline-block text-right fw-bold mr-3 p-0">Return Items: <span className="d-inline-block text-left fw-bolder mr-3 p-0">{Array.isArray(SellMapReturn) && SellMapReturn.length}</span></span>
...
export default connect(mapStateToProps, { logout })(PreviewInvoice);