...
const RetailSell = ({ user, list, setList }) => {
    ...
    const calculatePayment = (cash, bank) => {
        const newCash = parseFloat(cash || 0);
        const newBank = parseFloat(bank || 0);

        if (isNaN(newCash) || isNaN(newBank)) {
            alert("Invalid input: Cash and Bank values must be valid numbers.");
            return;
        }

        let paid = newCash + newBank;
        paid = paid.toFixed(2);
        setPaid(paid);
    ...
    const PaidCalc = (e) => {
        ...
        let payment;
        if (newCash && newBank) {
            payment = { label: "Partial", value: 19 };
        } else if (newCash) {
            payment = { label: "Paid in cash", value: 14 };
        } else if (newBank) {
            payment = { label: "Paid in bank", value: 18 };
        }
                        ...
                        <form className="row justify-content-center mx-2 py-2 px-0 my-0 h-100">
                            <div className='justify-content-start align-items-between m-0 p-0'>
                                ...
                                <div className="row justify-content-between p-0">
                                    ...
                                    <div className="col-md-6">
                                        <p className="text-center fs-3 text-dark fw-bold m-0 border-bottom">Quantity</p>
                                        <input
                                            ref={QtyFocus}
                                            type="number"
                                            className="form-control fs-3 fw-bold p-0 text-center"
                            ...
                            <div className='justify-content-center align-self-end my-2 mx-0 p-0'>
                                <div className="row justify-content-between p-0 align-items-end">
                                    <div className="col-md-6">
                                        <p className="text-center fs-3 text-dark fw-bold m-0 border-bottom">Cash</p>
                                        <input
                                            ref={CashFocus}
                                            type="text"
                                            id="Cash"
                                    ...
                                    <div className="col-md-6">
                                        <div className="row justify-content-between p-0 m-0" disabled>
                                            <p className="text-center fs-3 text-dark fw-bold m-0 border-bottom">Card</p>
                                            <div className="input-group fs-3 fw-bold p-0">
                                                <input
                                                    ref={BankFocus}
                                                    type="text"
                            ...
                            {
                                user.Collocation.Category === 2 || user.Collocation.Category === 3 ?
                                    <div className={`row m-0 p-0 ${WalkIN ? "border border-light rounded" : null}`}>
                                        <div className={`d-flex justify-content-center align-items-center`} style={{ borderRadius: "15px" }}>
                                            <div className="form-check form-switch">
                                                <input
                                                    className="form-check-input"
                                                    type="checkbox"
                                                ...
                                                />
                                                <label className="form-check-label" for="Walk">{WalkIN ? "Party" : "Walk-in"}</label>
                                            </div>
                                        </div>
...
export default connect(mapStateToProps, { logout })(RetailSell);