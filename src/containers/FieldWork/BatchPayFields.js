...
import { FetchInitData, LoadFarmStatus, RemoveInitialization } from '../../actions/ContractAPI';
...
const BatchPayFields = ({ display, list, setList, scale, sub_scale }) => {
    ...
    const MyFarms = async () => {
        ...
        var result = await LoadFarmStatus();
    ...
    FilterFarms = Array.isArray(Data) && Data.length ? Data.filter(function (item) {
        ...
        return BothValue
    }).map(function ({ id, FarmID, BatchID, Title, IssueDate, Sell, Status, UpdatedAt, UpdatedBy, Dispatched, details }) {
        return { id, FarmID, BatchID, Title, IssueDate, Sell, Status, UpdatedAt, UpdatedBy, Dispatched, details };
    }) : null
                            ...
                            Array.isArray(FilterFarms) && FilterFarms.length ?
                                FilterFarms.map((item, i) => (
                                    <div className={View ? "item col-xs-3 col-lg-3 grid-group-item mb-3" : "item col-xs-3 col-lg-3 grid-group-item list-group-item mb-3"} key={i}>
                                        ...
                                        <div className="justify-content-center align-items-center box thumbnail card py-2 shadow-none m-0 h-100">

                                            <div className="img-event d-flex flex-column justify-content-center align-items-center h-100" style={{ minHeight: "8vh" }}>
                                                <img src={item ? item.details.Image : process.env.REACT_APP_API_URL + "/Media/no_logo.jpeg"}
                                                    className="img-fluid mx-auto" alt="avatar"
                                            ...
                                            <div className="caption card-body d-flex flex-column justify-content-center py-0 px-2" style={{ minHeight: "15vh" }}>
                                                <div className="group inner list-group-item-text m-0">
                                                    <div className="group card-title inner list-group-item-text fs-4 fw-bold m-0">{item.FarmID + ". " + item.Title}</div>
                                                    <p className="group card-title inner list-group-item-text fw-bold m-0">{item.details.UserID + ". " + item.details.FullName}</p>
                                                    <small className="text-muted">{moment(item.IssueDate).format("DD MMM YYYY") + " | " + moment(item.Dispatched).format("DD MMM YYYY") + " (" + item.UpdatedBy + ")"}</small>
                                                </div>
                                                <small className='text-dark bg-light m-0 px-1 rounded fw-bold w-auto'>{item.Status === 0 ? "Batch Closed (" + moment(item.UpdatedAt).format("DD MMM YYYY hh:ss A") + ")" : item.Status === 1 ? "Active (" + moment(item.UpdatedAt).format("DD MMM YYYY hh:ss A") + ")" : item.Status === 2 ? "Request for batch review (" + moment(item.UpdatedAt).format("DD MMM YYYY hh:ss A") + ")" : item.Status === 3 ? "Waiting for accounts approval (" + moment(item.UpdatedAt).format("DD MMM YYYY hh:ss A") + ")" : item.Status === 4 ? "Paid (" + moment(item.UpdatedAt).format("DD MMM YYYY hh:ss A") + ")" : item.Status === 5 ? "Payment Hold (" + moment(item.UpdatedAt).format("DD MMM YYYY hh:ss A") + ")" : null}</small>
                                                {item.Status >= 3 ?
                                                    <Fragment>
                                                        <br />
                                                        <Link Title="Batch Account" style={{ borderRadius: "15px" }} className="text-success fw-bold mr-2 px-1" to={`/fwr_batch_account/${item.FarmID}`} >Batch Account</Link>
                                                    </Fragment>
                                                    : null}
                                            </div>
...
export default connect(mapStateToProps, { logout })(BatchPayFields);