...
import { LoadMyFarms, BatchPro, SaveScale, SellRunningTotal, ParkedBatchPro } from '../../../actions/ContractAPI';
...
const ParkedBirdSell = ({ display, BatchID, BusinessID, InvoiceNo, InvoiceID, list, setList }) => {
    ...
    const SendScale = async () => {
        if (Pices !== "" && Weight !== "") {
            dispatch({ type: DISPLAY_OVERLAY, payload: false });
            setWait(true);
            const result = await SaveScale(BatchID, InvoiceID, Pices, Weight);
            if (result !== true) {
...
export default connect(mapStateToProps, { logout })(ParkedBirdSell);