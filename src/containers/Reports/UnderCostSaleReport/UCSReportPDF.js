...
export const UCSReportPDF = async (e, table, date_to, user) => {
    ...
    doc.setFontSize(15).setTextColor(0, 0, 0).setFont(undefined, 'bold').text("Under Cost Sales Report", Left, 40)
    const dateRangeText = `Date- ${moment(date_to).format("DD MMM YYYY")}`; // If the dates are the same, set to an empty string
    doc.setFontSize(11).setTextColor(0, 0, 0).setFont(undefined, 'italic').text(dateRangeText, Left, 60)
    const headers = [["S/N", "CODE", "PRODUCT NAME", "UNIT NAME", "UNIT QTY", "PURCHASE", "SALE"]];
    ...
    let options = {
        ...
        didDrawPage: function (data) {
            ...
            if (pageCount !== 1) {
                ...
                doc.setFontSize(20);
                doc.setTextColor(40);
                doc.setFontSize(12).setFont(undefined, 'bold').text("Product Sold Out Items", data.settings.margin.left, 40, { align: "left" })
    ...
    const fileName = `Under Cost Sales Report ${dateRangeText}.pdf`
