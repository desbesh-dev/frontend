...
import { FetchSoldOutItemPDF, MyProductList } from '../../../actions/InventoryAPI';
import { DISPLAY_OVERLAY } from '../../../actions/types';
import '../../../hocs/react-select/dist/react-select.css';
import { customHeader, locales } from "../../Suppliers/Class/datepicker";
import { SoldOutItemsPDF } from "./SoldOutItemsPDF";

let today = new Date();
const oneMonthAgo = new Date();
oneMonthAgo.setMonth(oneMonthAgo.getMonth() - 1);

const SoldOutItems = ({ user, no }) => {
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/sold_items_list/`, {
                headers: {
    ...
    const PrintPDF = async (e) => {
        e.preventDefault();
        dispatch({ type: DISPLAY_OVERLAY, payload: true });
        var result = await FetchSoldOutItemPDF(itemsPerPage, DateTo, SectorFilter, searchKey);

        if (result !== true) {
            SoldOutItemsPDF(e, result.data.results, DateTo, user)
            dispatch({ type: DISPLAY_OVERLAY, payload: false });
        }
        dispatch({ type: DISPLAY_OVERLAY, payload: false });
    }
            ...
            <div className="col-lg-12 px-0">
                <div className="row d-flex bg-white mx-auto mb-1 py-1">
                    <div className={`d-flex justify-content-between align-items-center bg-white`}>
                        <p className='fs-3 bg-white fw-bolder text-dark text-nowrap m-0 w-25'>SOLD OUT ITEMS</p>
                        ...
                        <button className="btn fs-3 px-2 ml-2 py-0 text-dark border-left"
                            // onClick={(e) => SoldOutItemsPDF(e, '#OrderedTable', DateFrom, DateTo, user)}
...
export default connect(mapStateToProps, { logout })(SoldOutItems);