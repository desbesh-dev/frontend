...
import { FetchSupplierAgedInvoicesList } from '../../../actions/SuppliersAPI';
import { DISPLAY_OVERLAY } from '../../../actions/types';
import { customHeader, locales } from "../../Suppliers/Class/datepicker";
import { SupplierAgedInvoicePDF } from './SupplierAgedInvoicePDF';
let today = new Date();

const SupplierAgedInvoices = ({ data, no }) => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/sup_aged_invoice_list/`, {
                headers: {
    ...
    const LoadAgedInvoices = async (e) => {
        e.preventDefault();
        dispatch({ type: DISPLAY_OVERLAY, payload: true });
        var result = await FetchSupplierAgedInvoicesList(moment(DateFrom).format("YYYY-MM-DD"), moment(DateTo).format("YYYY-MM-DD"), SisterFilter, SectorFilter, sharpnessValue, OrderBy);

        if (result !== true)
            SupplierAgedInvoicePDF(e, result.data, false, DateFrom, DateTo, data.Name, SisterFilter?.label + " (" + SectorFilter?.label + ")", sharpnessValue?.label, OrderBy?.label)
            ...
            <div className="col-md-12 justify-content-center align-items-center h-100 p-0">
                <div className={`d-flex justify-content-between bg-white py-2 px-2`}>
                    ...
                    <div className="d-flex">
                        <p className='display-6 bg-white fw-bolder m-0 text-nowrap pr-3'>SUPPLIER AGED INVOICES</p>
                        <Datepicker
                            selected={DateFrom}
                    ...
                    <div className="d-flex justify-content-end">
                        ...
                        <div className="d-flex justify-content-center mx-2 w-70">
                            <Select
                                ...
                                borderRadius={"0px"}
                                options={[
                                    { label: "Supplier Title (ASC)", value: 1 },
                                    { label: "Supplier Title (DESC)", value: 2 },
                        ...
                        <button className="btn fs-3 px-2 ml-2 py-0 text-dark border-left" onClick={(e) => LoadAgedInvoices(e)}><i className="fad fa-file-pdf"></i></button>
                    </div>
                </div>
                {Data && Data.results &&
                    <>
                                ...
                                <tbody>
                                    {
                                        Array.isArray(Data.results) && Data.results.length ? Data.results.map((item, i, Data) => (
                                            <tr className="border-bottom text-center" key={i}>
                                                <td className="border-right py-1 px-2"><span className="fw-bold text-center text-dark text-nowrap"> {(currentPage - 1) * 50 + (i + 1)}</span> </td>
                                                <td className="border-right py-1 px-2"><span className="fw-bold text-center text-dark text-nowrap">{moment(item.Date).format("DD MMM YYYY")}</span> </td>
                                                <td className="border-right py-1 px-2"><span className="d-block fs-6 fw-bold text-center text-dark px-2" >{item.InvoiceNo === "" ? "Docket" : item.InvoiceNo}</span></td>
                                                <td className="border-right py-1 px-2 text-left"><Link className="fs-6 fw-bolder text-left text-dark text-decoration-none" to={`/my_supplier/${item.SupplierID}`}>{item.PartyTitle}</Link> <br /> <small className='p-0 m-0 text-left'>{item.Address} </small></td>
...
export default connect(mapStateToProps, { logout })(SupplierAgedInvoices);