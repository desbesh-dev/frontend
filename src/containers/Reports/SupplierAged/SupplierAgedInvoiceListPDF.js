...
export const SupplierAgedInvoiceListPDF = async (e, item, date_from, date_to, user, SectorFilter) => {
    e.preventDefault();
    ...
    doc.setFontSize(15).setTextColor(0, 0, 0).setFont(undefined, 'bold').text("Supplier Ageing Reports", 820, 40, { align: "right" })
    ...
    const headers = [["S/N", "Title", "Date", "Received", "Invoice No", "Days", "Status", "Paid", "Due", "0 - 30 days", "31 - 60 days", "61 - 90 days", "90 + days"]];
    ...
    const body = item.map((item, i) => {
        ...
        return [
            ...
            item.InvoiceNo === "" ? "Docket" : item.InvoiceNo,
            daysSinceInvoice,
    ...
    let options = {
        ...
        didDrawPage: function (data) {
            ...
            if (pageCount !== 1) {
                ...
                doc.setFontSize(10).setTextColor(105, 105, 105).setFont("helvetica", 'normal').text("Supplier Ageing Reports (" + dateRangeText + ")", data.settings.margin.left, 50, { align: "left" });
            }
            // Footer
            doc.setFontSize(10);
        }
    ...
    const fileName = `All Supplier Aged Invoices ${moment(item.InvDate).format("DD MMM YYYY")}`
