...
import { FetchPurchaseInvoice, FetchSupplierAgedInvoicesList } from '../../../actions/SuppliersAPI';
...
import { SupplierAgedInvoiceListPDF } from './SupplierAgedInvoiceListPDF';
let today = new Date();
const oneMonthAgo = new Date();
oneMonthAgo.setMonth(oneMonthAgo.getMonth() - 1);

const SupplierAgedInvoiceList = ({ user, SupplierID, SupplierData, no }) => {
    ...
    const AgedInvoiceList = async () => {
        ...
        var result = await FetchSupplierAgedInvoicesList(date_from, date_to, SectorFilter?.value);
        if (result !== true)
            setData(result.data);
    ...
    return (
        <div className="col-md-12 justify-content-center align-items-center px-2">
            <div className={`d-flex justify-content-between bg-white py-1 px-0`}>
                <p className='display-6 bg-white fw-bolder m-0 px-2'>Supplier Ageing Reports</p>
                <div className="d-flex justify-content-end align-items-center bg-white p-0 w-75">
                    ...
                    <button className="btn fs-3 px-2 ml-2 py-0 text-dark border-left" onClick={(e) => SupplierAgedInvoiceListPDF(e, Data, DateFrom, DateTo, user, SisterFilter?.label + " (" + SectorFilter?.label + ")",)}><i className="fad fa-file-pdf"></i></button>
...
export default connect(mapStateToProps, { logout })(SupplierAgedInvoiceList);
