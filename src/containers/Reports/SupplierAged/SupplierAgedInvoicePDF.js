...
export const SupplierAgedInvoicePDF = async (e, item, status, date_from, date_to, user, SectorFilter, sharpnessValue, OrderBy) => {
    ...
    doc.setFontSize(16).setTextColor(0, 0, 0).setFont('helvetica', 'bold').text("SUPPLIER AGED INVOICES", x, 90, { align: "center" });
    ...
    const headers = [["S/N", "DATE", "SUPPLIER TITLE", "CONTACT", "AMOUNT", "AGE"]];

    // Calculate total invoices, unique parties, total amount, and max age
    const totalInvoices = item.results.length; // Total invoices
    const uniqueParties = [...new Set(item.results.map((i) => i.PartyTitle))].length; // Unique Party titles
    const totalAmount = item.results.reduce((sum, item) => sum + parseFloat(item.Due), 0).toLocaleString("en-BD", { minimumFractionDigits: 2 }); // Total amount
    const maxAge = Math.max(...item.results.map((item) => item.age)); // Maximum age

    // Map the body of the table
    ...
    body.push([
        totalInvoices, // S/N column empty
        "", // Label for total
        `TOTAL PARTY ${uniqueParties}`, // Unique parties count
    ...
    let options = {
        ...
        columnStyles: {
            ...
            1: { cellWidth: 90, valign: 'middle', halign: 'center' },
            2: { cellWidth: 'auto', valign: 'middle', halign: 'left', fontSize: 9 },
            3: { cellWidth: 70, valign: 'middle', halign: 'left', fontSize: 9 },
            4: { cellWidth: 70, valign: 'middle', halign: 'right', fontSize: 9 },
            5: { cellWidth: 95, valign: 'middle', halign: 'center' },
        },
        rowStyles: {
