...
import { FetchOrderedItemPDF, MyProductList } from '../../../actions/InventoryAPI';
import { DISPLAY_OVERLAY } from '../../../actions/types';
import '../../../hocs/react-select/dist/react-select.css';
import { customHeader, locales } from "../../Suppliers/Class/datepicker";
import { OrderedItemsPDF } from "./OrderedItemsPDF";

let today = new Date();
const oneMonthAgo = new Date();
oneMonthAgo.setMonth(oneMonthAgo.getMonth() - 1);

const OrderedItems = ({ user, no }) => {
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/ordered_items_list/`, {
                headers: {
    ...
    const PrintPDF = async (e) => {
        e.preventDefault();
        dispatch({ type: DISPLAY_OVERLAY, payload: true });
        var result = await FetchOrderedItemPDF(itemsPerPage, DateFrom, DateTo, SectorFilter, searchKey);

        if (result !== true) {
            OrderedItemsPDF(e, result.data.results, DateFrom, DateTo, user)
            dispatch({ type: DISPLAY_OVERLAY, payload: false });
        }
        dispatch({ type: DISPLAY_OVERLAY, payload: false });
    }
            ...
            <div className="col-lg-12 px-0">
                <div className="row d-flex bg-white mx-auto mb-1 py-1">
                    <div className={`d-flex justify-content-between align-items-center bg-white`}>
                        <p className='fs-3 bg-white fw-bolder text-dark text-nowrap m-0 w-25'>ORDERED ITEMS</p>
                        ...
                        <button className="btn fs-3 px-2 ml-2 py-0 text-dark border-left"
                            // onClick={(e) => OrderedItemsPDF(e, '#OrderedTable', DateFrom, DateTo, user)}
                ...
                {filteredResults && filteredResults.length ?
                    loading ? (
                        ...
                        <div className='tableFixHead w-100' style={{ height: h + "px" }}>
                            <table id='OrderedTable' className={`table table-hover table-borderless`}>
                                <thead>
                                    <tr className="text-center text-uppercase bg-white">
                                        ...
                                        <th className="p-1 border-right"><span className="d-block text-center fw-bolder">Ctn/Bale/Bag</span></th>
                                        <th className="p-1 border-right"><span className="d-block text-center fw-bolder">Qty</span></th>
                                        <th className="p-1"><span>Weight</span></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {
                                        filteredResults?.map((item, i) => (
                                            <tr className="border-bottom text-center" key={i}>
                                                ...
                                                <td className="p-0 border-right"><span className="d-block fw-bold text-right px-1">{parseFloat(item.Qty / item.CtnQty).toLocaleString("en", { minimumFractionDigits: 2 })}</span> </td>
                                                ...
                                                <td className="p-0"> <span className="d-block fw-bold text-right px-1">{parseFloat(item.Weight).toLocaleString("en", { minimumFractionDigits: 3 })}</span>  </td>
                                            </tr>
                                        ))
                                    }
                                </tbody>
...
export default connect(mapStateToProps, { logout })(OrderedItems);