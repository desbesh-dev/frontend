...
export const PartyAgedInvoiceListPDF = async (e, item, date_from, date_to, user, SectorFilter) => {
    e.preventDefault();
    ...
    doc.setFontSize(15).setTextColor(0, 0, 0).setFont(undefined, 'bold').text("Party Ageing Reports", 820, 40, { align: "right" })
    ...
    doc.setFontSize(9).setTextColor(0, 0, 0).setFont(undefined, 'normal').text("Print Time & Date: " + date, 820, 64, { align: "right" })
    ...
    const headers = [["S/N", "Title", "Date", "Order No", "Invoice No", "Days", "Status", "Paid", "Due", "0 - 30 days", "31 - 60 days", "61 - 90 days", "90 + days"]];
    ...
    let options = {
        ...
        didDrawPage: function (data) {
            ...
            if (pageCount !== 1) {
                ...
                doc.setFontSize(10).setTextColor(105, 105, 105).setFont("helvetica", 'normal').text("Party Ageing Reports (" + dateRangeText + ")", data.settings.margin.left, 50, { align: "left" });
            }
            // Footer
            doc.setFontSize(10);
        }
    ...
    const fileName = `All Party Aged Invoices ${moment(item.Date).format("DD MMM YYYY")}`
