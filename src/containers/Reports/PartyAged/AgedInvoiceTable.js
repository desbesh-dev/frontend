...
const InvoiceTable = ({ Data, PrintPDF, sendTableIdToParent }) => {
    ...
    return (
        <table id={tableId} className="table table-hover table-borderless bg-white">
            ...
            <tbody>
                {Data.map((item, i) => {
                    const { Title, PaymentStatus, PaidAmount, Due, Date, OrderNo, InvoiceNo, Payment, Party, PartyID, id } = item;
                    const daysSinceInvoice = moment().diff(moment(Date), 'days');
                    ...
                    return (
                        <tr className="border-bottom text-center fw-bold" key={id}>
                            ...
                            <td className="border-right py-1 px-2 text-left"><Link className="fs-6 fw-bolder text-left text-dark text-decoration-none" to={`/my_party/${item.Party}/${item.PartyID}`}>{Title}</Link></td>
                            ...
                            <td className="py-0 px-1 border-right">{({ 1: "Partial Paid", 2: "Paid", 3: "Unpaid" }[PaymentStatus] || "N/A")}</td>
                            ...
                            <td className="p-0 text-nowrap text-center">
                                <Link className="btn fs-3 px-2 py-0 text-danger" to={`/sell_invoice_preview/${item.id}`}>
