...
const Navbar = ({ logout, user, scale, no, cat }) => {
	...
	const authLinks = () => (
		<Fragment>
			...
			<li className='nav-item mx-2'>
				<Link title='Shrink' className='nav-link' to='home'><i className="fad fa-compress-arrows-alt fs-3 fa-fw"></i></Link>
			</li>
			...
			<li className="nav-item dropdown" onBlur={(e) => GridMenu(e)}>
				...
				<ul className={`dropdown-menu dropdown-menu-right mx-2 px-2 ${Grid ? "show" : null}`} aria-labelledby="grid_menu" style={{ zIndex: "1000", height: "auto", maxHeight: "420px", width: "350px", overflowX: "hidden" }}>
					{no === 8 && cat === 4 ?
						<li className='m-2'>
	...
	return (
		<nav className="navbar navbar-expand-lg navbar-theme bg-white rounded py-0 mb-2 border-bottom px-2" aria-label="Eleventh navbar example">
			...
			{/* NavBar ?
							<img src={logo} className="img-fluid mx-auto d-table" width="120" height="40" alt="SoftaPoul" />
							: */}
			<Link
